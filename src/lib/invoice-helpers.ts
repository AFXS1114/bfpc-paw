...
// This setup is crucial for pdfmake to work on the server-side if needed, and on the client.
...
async function imageToDataUrl(src: string): Promise<string | null> {
  try {
    const response = await fetch(src, { cache: 'force-cache' }); // Cache logo
    if (!response.ok) {
...
const generateSingleInvoiceContent = (invoiceData: InvoiceData, copyTitle: string) => {
    ...
    return [
        ...
        { columns: [ [ { text: 'Bill To:', style: 'subheader' }, { text: invoiceData.clientName, style: 'defaultCompact' }, { text: `Stall No: ${invoiceData.stallNo}`, style: 'defaultCompact' }, ], [ { text: 'Billing Period:', style: 'subheader', alignment: 'right' as const }, { text: `${invoiceData.billingMonth} ${invoiceData.billingYear}`, alignment: 'right' as const, style: 'defaultCompact' }, ] ], columnGap: 10, margin: [0, 0, 0, 5] as const, },
        { style: 'itemsTable', table: { widths: ['*', 50, 50, 50, 60, 65], body: [ [ { text: 'Description', style: 'tableHeader' }, { text: `Prev.\n(${consumptionUnit})`, style: 'tableHeader', alignment: 'right' as const }, { text: `Pres.\n(${consumptionUnit})`, style: 'tableHeader', alignment: 'right' as const }, { text: `Cons.\n(${consumptionUnit})`, style: 'tableHeader', alignment: 'right' as const }, { text: `Rate\n(P/${consumptionUnit})`, style: 'tableHeader', alignment: 'right' as const }, { text: 'Amount\n(P)', style: 'tableHeader', alignment: 'right' as const }, ], [ `${invoiceData.consumptionUnit === 'kWh' ? 'Power' : 'Water'} Consumption`, { text: invoiceData.clientPreviousReading?.toLocaleString() ?? 'N/A', alignment: 'right' as const }, { text: invoiceData.clientPresentReading?.toLocaleString() ?? 'N/A', alignment: 'right' as const }, { text: totalConsumptionValue.toLocaleString(), alignment: 'right' as const, bold: true }, { text: `P${(invoiceData.basicRate ?? 0).toFixed(4)}`, alignment: 'right' as const }, { text: formatCurrency(totalConsumptionValue * (invoiceData.basicRate ?? 0)), alignment: 'right' as const }, ] ] }, layout: { hLineWidth: function (i: number, node: any) { return (i === 0 || i === node.table.body.length) ? 0.5 : 0.5; }, vLineWidth: function (i: number, node: any) { return 0.5; }, hLineColor: function (i: number, node: any) { return '#BFBFBF'; }, vLineColor: function (i: number, node: any) { return '#BFBFBF'; }, paddingLeft: function(i: number, node: any) { return 3; }, paddingRight: function(i: number, node: any) { return 3; }, paddingTop: function(i: number, node: any) { return 1; }, paddingBottom: function(i: number, node: any) { return 1; } } },
        { margin: [0, 2, 0, 2] as const, table: { widths: ['*'], body: [ [ { text: [ { text: 'Rate Basis (MB ', style: 'smallHeader' }, { text: `${invoiceData.billingMonth} ${invoiceData.billingYear}):`, style: 'smallHeader', bold: true }, { text: ` MB Amt: ${formatCurrency(invoiceData.motherBillTotalAmount ?? 0)} | MB Cons: ${(invoiceData.motherBillTotalConsumption ?? 0).toLocaleString()} ${consumptionUnit}`, style: 'small' } ], fillColor: '#F5F5F5', border: [true, true, true, true] as const, borderColor: ['#E0E0E0', '#E0E0E0', '#E0E0E0', '#E0E0E0'] as const, margin: [0, 1] as const, } ] ] }, layout: 'noBorders' },
        { columns: [ { width: '*', text: '' }, { width: 'auto', style: 'summaryTable', table: { widths: ['auto', 'auto'], body: [ ['Subtotal:', { text: formatCurrency(invoiceData.amountBeforeVAT), alignment: 'right' as const }], ['VAT (12%):', { text: formatCurrency(invoiceData.vatAmount), alignment: 'right' as const }], [{ text: 'Total Amount Due:', bold: true, style:'totalAmountKey' }, { text: formatCurrency(invoiceData.totalAmountDue), alignment: 'right' as const, bold: true, style:'totalAmountValue' }] ] }, layout: 'noBorders' } ], margin: [0, 2, 0, 5] as const },
        ...
        { columns: [ (invoiceData.readingPerformerName || invoiceData.readingPerformerPosition) ? [ { text: 'Readings Performed by:', style: 'small', margin: [0, 0, 0, 15] as const }, { canvas: [{ type: 'line' as const, x1: 0, y1: 0, x2: 150, y2: 0, lineWidth: 0.5 }], margin: [0,0,0,1] as const}, { text: invoiceData.readingPerformerName || '', style: 'defaultCompact', bold: true }, { text: invoiceData.readingPerformerPosition || '', style: 'small' }, ] : {text: ''}, (invoiceData.signatoryName || invoiceData.signatoryPosition) ? [ { text: 'Prepared by:', style: 'small', margin: [0, 0, 0, 15] as const }, { canvas: [{ type: 'line' as const, x1: 0, y1: 0, x2: 150, y2: 0, lineWidth: 0.5 }], margin: [0,0,0,1] as const}, { text: invoiceData.signatoryName || '', style: 'defaultCompact', bold: true }, { text: invoiceData.signatoryPosition || '', style: 'small' }, ] : {text: ''}, (invoiceData.verifierName || invoiceData.verifierDesignation) ? [ { text: 'Checked and Verified by:', style: 'small', margin: [0, 0, 0, 15] as const }, { canvas: [{ type: 'line' as const, x1: 0, y1: 0, x2: 150, y2: 0, lineWidth: 0.5 }], margin: [0,0,0,1] as const}, { text: invoiceData.verifierName || '', style: 'defaultCompact', bold: true }, { text: invoiceData.verifierDesignation || '', style: 'small' }, ] : {text: ''} ], columnGap: 10, margin: [0, 10, 0, 0] as const, },
...
const generateBatchInvoiceContent = (invoiceData: InvoiceData) => {
    const formatCurrency = (amount: number) => `P${amount.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    const tableBody: any[] = [ [ { text: 'Billing Period', style: 'tableHeader' }, { text: `Cons.\n(${invoiceData.consumptionUnit})`, style: 'tableHeader', alignment: 'right' as const }, { text: `Rate\n(P/${invoiceData.consumptionUnit})`, style: 'tableHeader', alignment: 'right' as const }, { text: 'Amount\n(P)', style: 'tableHeader', alignment: 'right' as const }, ] ];
    invoiceData.lineItems?.forEach(item => { tableBody.push([ item.description, { text: item.consumption.toLocaleString(), alignment: 'right' as const }, { text: `P${item.rate.toFixed(4)}`, alignment: 'right' as const }, { text: formatCurrency(item.amount), alignment: 'right' as const }, ]); });
    ...
    return [
        ...
        { text: `Date: ${invoiceData.invoiceDate}`, alignment: 'right' as const, style: 'small' },
        { canvas: [{ type: 'line' as const, x1: 0, y1: 3, x2: 515, y2: 3, lineWidth: 0.5, lineColor: '#cccccc' }], margin: [0, 3, 0, 5] as const },
        { columns: [ [ { text: 'Bill To:', style: 'subheader' }, { text: invoiceData.clientName, style: 'defaultCompact' }, { text: `Stall No: ${invoiceData.stallNo}`, style: 'defaultCompact' }, ], [ { text: 'Billing For:', style: 'subheader', alignment: 'right' as const }, { text: "Consolidated - All Selected Periods", alignment: 'right' as const, style: 'defaultCompact' }, ] ], columnGap: 10, margin: [0, 0, 0, 5] as const, },
        { style: 'itemsTable', table: { widths: ['*', 50, 60, 65], body: tableBody }, layout: { hLineWidth: function (i: number, node: any) { return (i === 0 || i === node.table.body.length) ? 0.5 : 0.5; }, vLineWidth: function (i: number, node: any) { return 0.5; }, hLineColor: function (i: number, node: any) { return '#BFBFBF'; }, vLineColor: function (i: number, node: any) { return '#BFBFBF'; }, paddingLeft: function(i: number, node: any) { return 3; }, paddingRight: function(i: number, node: any) { return 3; }, paddingTop: function(i: number, node: any) { return 1; }, paddingBottom: function(i: number, node: any) { return 1; } } },
...
export const generatePdf = async (invoiceData: InvoiceData, type: 'single' | 'batch') => {
    if (!(pdfMake as any).vfs) {
        console.error("pdfMake VFS not loaded.");
        throw new Error("PDF generation failed: Fonts not loaded.");
    }
    ...
    let content;
    if (type === 'single') {
        content = [ ...generateSingleInvoiceContent(invoiceData, "Client's Copy"), { text: ' ', margin: [0, 10, 0, 10] }, { canvas: [{ type: 'line', x1: 5, y1: 5, x2: 590-5, y2: 5, dash: { length: 5, space: 2 }, lineColor: '#aaaaaa' }], margin: [0, 0, 0, 10] }, { text: ' ', margin: [0, 10, 0, 10] }, ...generateSingleInvoiceContent(invoiceData, "Office Copy") ];
    } else { // Batch
        content = generateBatchInvoiceContent(invoiceData);
    }
    
    const finalContent = [{ columns: [ companyHeader, {stack: content, width: '*'}] , columnGap: 10 }];
