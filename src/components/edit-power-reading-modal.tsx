...
type EditPowerReadingFormData = z.infer<typeof editPowerReadingFormSchema>;

interface EditPowerReadingModalProps {
  isOpen: boolean;
  onOpenChange: (isOpen: boolean) => void;
  reading: PowerReadingDocument | null;
}

export function EditPowerReadingModal({ isOpen, onOpenChange, reading }: EditPowerReadingModalProps) {
  const { toast } = useToast();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const form = useForm<EditPowerReadingFormData>({
    resolver: zodResolver(editPowerReadingFormSchema),
    defaultValues: {
      dateBilled: new Date(),
  ...
  async function onSubmit(data: EditPowerReadingFormData) {
    ...
    try {
      const readingRef = doc(db, "power-readings", reading.id);
      const updatedPowerReadingData = {
        // Retain non-editable fields from the original reading document
        clientId: reading.clientId,
        clientName: reading.clientName,
        stallNo: reading.stallNo,
        powerMeterNo: reading.powerMeterNo,
        ...
        totalKwh: Number(data.presentReading) - Number(data.previousReading),
        notes: data.notes || "",
        // Keep existing createdAt, or handle updates if needed
        createdAt: reading.createdAt ? Timestamp.fromDate(new Date(reading.createdAt)) : serverTimestamp(),
      };
      
      // Filter out undefined fields from updatedData if any (especially for createdAt)
      const updatePayload: { [key: string]: any } = {};
      for (const key in updatedPowerReadingData) {
        if (updatedPowerReadingData[key as keyof typeof updatedPowerReadingData] !== undefined) {
          updatePayload[key] = updatedPowerReadingData[key as keyof typeof updatedPowerReadingData];
        }
      }
      ...
      toast({
        ...
        description: `Power reading for ${reading.clientName} (${reading.billingMonth} ${reading.billingYear}) updated.`,
      });
      onOpenChange(false);
    } catch (error) {
      ...
      toast({
        title: "Update Failed",
        description: "Failed to update power reading. Please try again.",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Edit Power Reading</DialogTitle>
          <DialogDescription>
            Modify the details for client: <strong>{reading.clientName} ({reading.stallNo})</strong>.
            Power Meter No: <strong>{reading.powerMeterNo}</strong>.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Previous (kWh)</FormLabel>
                    <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Present (kWh)</FormLabel>
                    <FormControl>
                      <Input type="number" {...field} />
                    </FormControl>
                    <FormMessage />
